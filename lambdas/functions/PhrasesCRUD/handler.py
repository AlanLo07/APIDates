...
def lambda_handler(event, context):
    ...
    logger.info(json.dumps({"level": "⚪️", "message": "Solicitud PhrasesCRUD", "method": method, "path": raw_path, "has_id": bool(item_id), "function": getattr(context, "function_name", "unknown")}, ensure_ascii=False))

    if method == "OPTIONS":
        logger.info(json.dumps({"level": "🟢", "message": "Preflight CORS PhrasesCRUD"}, ensure_ascii=False))
    ...
    try:
        ...
        logger.warning(json.dumps({"level": "🟡", "message": "Validación fallida en PhrasesCRUD", "error": str(e)}, ensure_ascii=False))
        return build_response(400, {"error": str(e)})
    except ClientError as e:
        logger.error(json.dumps({"level": "🔴", "message": "Error DynamoDB en PhrasesCRUD", "code": e.response.get("Error", {}).get("Code", "Unknown")}, ensure_ascii=False))
        return build_response(502, {"error": "Error de base de datos"})
    except Exception:
        logger.exception("🔴 Error inesperado en PhrasesCRUD")
